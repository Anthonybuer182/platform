...
import (
	...
	"platform/internal/counter/domain"
...
func (g *counterGRPCServer) GetListOrderFulfillment(
	ctx context.Context,
	request *gen.GetListOrderFulfillmentRequests,
) (*gen.GetListOrderFulfillmentResponses, error) {
	slog.Info("GET: GetListOrderFulfillment")

	res := gen.GetListOrderFulfillmentResponses{}
	...
	for _, entity := range entities {
		res.Orders = append(res.Orders, &gen.OrderDtos{
			...
			LineItems: lo.Map(entity.LineItems, func(item *domain.LineItem, _ int) *gen.LineItemDtos {
				return &gen.LineItemDtos{
					Id:             item.ID.String(),
...
func (g *counterGRPCServer) PlaceOrder(
	ctx context.Context,
	request *gen.PlaceOrderRequests,
) (*gen.PlaceOrderResponses, error) {
